    ...
    Statement = [
      ...
      },
      {
        ...
        Action = [
          ...
          "bedrock:InvokeAgent", "bedrock:GetAgent", "bedrock:ListAgents", "bedrock:InvokeModel"
        ]
        Resource = "*"
      },
      {
...
resource "aws_ecs_task_definition" "dr" {
  ...
  container_definitions = jsonencode([
    {
      name      = each.key
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.dr_region}.amazonaws.com/${var.project_name}-${var.environment}-${each.key}:${data.aws_ecr_image.dr_latest[each.key].image_tags[0]}"
      essential = true
      ...
      environment = concat(
        ...
        each.key == "chatbot" ? [
          ...
          { name = "ANALYSIS_SERVICE_URL", value = "http://analysis.${var.project_name}-${var.environment}-dr.internal:8000" },
          { name = "USER_SERVICE_URL", value = "http://users.${var.project_name}-${var.environment}-dr.internal:8000" },
          { name = "USE_MEMORY", value = "true" },
          { name = "MEMORY_ID", value = data.aws_ssm_parameter.agentcore_memory_id.value },
          { name = "SUPERVISOR_AGENT_ARN", value = var.dr_supervisor_agent_arn },
        ] : [],
        each.key == "users" ? [
          { name = "S3_BUCKET_NAME", value = "${var.project_name}-${var.environment}-codef-data-dr" },
          { name = "CODEF_CLIENT_ID", value = var.codef_client_id },
          { name = "CODEF_CLIENT_SECRET", value = var.codef_client_secret },
        ...
        each.key == "analysis" ? [
          { name = "USER_SERVICE_URL", value = "http://users.${var.project_name}-${var.environment}-dr.internal:8000" },
          { name = "AGENTCORE_RUNTIME_ARN", value = var.dr_analysis_agent_arn },
        ] : [],
        [
          { name = "OTEL_SERVICE_NAME", value = "${var.project_name}-${var.environment}-${each.key}" },
      ...
      secrets = contains(["users", "history", "analysis", "chatbot"], each.key) ? [
        {
          name      = "DB_HOST"
          valueFrom = "arn:aws:ssm:${var.dr_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.project_name}/${var.environment}/rds/${each.key}-cluster/endpoint"
        },
        {
          name      = "DB_PORT"
          valueFrom = "arn:aws:ssm:${var.dr_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.project_name}/${var.environment}/rds/${each.key}-cluster/port"
        },
        {
          name      = "DB_NAME"
          valueFrom = "arn:aws:ssm:${var.dr_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.project_name}/${var.environment}/rds/${each.key}-cluster/dbname"
        },
        {
          name      = "DB_USER"
          valueFrom = "arn:aws:ssm:${var.dr_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.project_name}/${var.environment}/rds/${each.key}-cluster/username"
        },
        {
          name      = "DB_PASSWORD"
          valueFrom = "${data.aws_secretsmanager_secret.dr_cluster[each.key].arn}:password::"
        }
      ] : []
