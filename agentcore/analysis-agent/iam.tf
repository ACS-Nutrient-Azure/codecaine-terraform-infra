...
resource "aws_iam_role" "lambda_exec" {
  name = "${upper(var.project_name)}-${upper(var.environment)}-ANALYSIS-AGENT-LAMBDA-ROLE"
...
resource "aws_iam_role" "agentcore_runtime" {
  name = "${upper(var.project_name)}-${upper(var.environment)}-AGENTCORE-RUNTIME-ROLE"
...
resource "aws_iam_role" "github_actions" {
  name = "${upper(var.project_name)}-${upper(var.environment)}-ANALYSIS-AGENT-GITHUB-ACTIONS-ROLE"
