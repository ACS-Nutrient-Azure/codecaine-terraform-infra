locals {
  agent_name  = replace(lower("${var.project_name}_${var.environment}_analysis_agent"), "-", "_")
  image_uri   = data.terraform_remote_state.ecr.outputs.agent_repository_urls["analysis-agent"]
  ssm_arn_key = "/${var.project_name}/${var.environment}/agentcore/analysis-agent/runtime-arn"
}
...
resource "aws_lambda_invocation" "agentcore_runtime" {
  ...
  input = jsonencode({
    ...
    environment_variables = {
      LAMBDA_FUNCTION_NAME = lower("${var.project_name}-${var.environment}-nutrient-calc")
    }
  })
