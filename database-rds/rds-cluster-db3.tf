...
resource "aws_secretsmanager_secret" "aurora_cluster3" {
  name                    = "${var.project_name}-${var.environment}-${var.aurora_clusters["cluster3"].cluster_identifier}-password"
...
resource "aws_secretsmanager_secret_version" "aurora_cluster3" {
  secret_id = aws_secretsmanager_secret.aurora_cluster3.id
  secret_string = jsonencode({
    username = var.aurora_clusters["cluster3"].master_username
    password = var.aurora_clusters["cluster3"].master_password
...
resource "aws_rds_cluster" "aurora_cluster3" {
  ...
  master_password    = var.aurora_clusters["cluster3"].master_password
  port               = var.postgres_port
  ...
  serverlessv2_scaling_configuration {
    max_capacity = var.aurora_clusters["cluster3"].serverless_max_capacity
    min_capacity = var.aurora_clusters["cluster3"].serverless_min_capacity
  }
