  ...
  table_mappings = jsonencode({
    rules = [
...

      {
        ...
        value = "analysis_current_ingredients"
      },
      {
        rule-type   = "transformation"
        rule-id     = "302"
...
# ============================================================
# user_to_analysis 태스크에 user_condition_snapshots 추가
# user_condition_snapshots.status → analysis_userdata.ans_current_conditions
# ============================================================
    ...
    rules = [
      {
        rule-type   = "selection"
        rule-id     = "1"
        rule-name   = "sel-user-condition-snapshots"
        rule-action = "include"
        object-locator = {
          schema-name = "public"
          table-name  = "user_condition_snapshots"
        }
      },
      {
        rule-type   = "transformation"
        rule-id     = "101"
        rule-name   = "rename-table-user-condition-snapshots"
        rule-action = "rename"
        rule-target = "table"
        object-locator = {
          schema-name = "public"
      ...
      },
      {
        rule-type   = "transformation"
        rule-id     = "102"
        rule-name   = "rename-status-to-ans-current-conditions"
        rule-action = "rename"
        rule-target = "column"
        object-locator = {
          schema-name = "public"
        ...
        value = "ans_current_conditions"
      }
    ]
  })

  tags = {
    Name = "${local.name_prefix}-user-condition-to-analysis"
  }
}

# ============================================================
# Target Endpoint: chatbot-cluster
# ============================================================
...
# ============================================================
# Replication Task: users → chatbot
# user_profile → chatbot_userdata (chat_ prefix)
# user_condition_snapshots.status → chatbot_userdata.chat_current_conditions
# current_supplements → chatbot_supplements (chat_ prefix)
# current_ingredients → chatbot_current_ingredients (chat_ prefix)
# ============================================================
...
resource "aws_dms_replication_task" "users_to_chatbot" {
  ...
  table_mappings = jsonencode({
    rules = [
      ...
      },
      {
        rule-type   = "transformation"
        rule-id     = "202"
        rule-name   = "rename-status-to-chat-current-conditions"
        rule-action = "rename"
        rule-target = "column"
        object-locator = {
          schema-name = "public"
  ...
  tags = {
    Name = "${local.name_prefix}-users-to-chatbot"
  }
}

# ============================================================
# Source Endpoint: analysis-cluster (source for chatbot)
# ============================================================
...
# ============================================================
# Replication Task: analysis → chatbot
# analysis_result → chatbot_analysis_result (chat_ prefix)
# nutrient_gap → chatbot_nutrient_gap (chat_ prefix, nutrient_id 포함)
# recommendations → chatbot_recommendations (chat_ prefix)
# NOTE: nutrient_gap.nutrient_id → nutrients.unit JOIN은 DMS 미지원
#       chatbot_nutrient_gap에 chat_nutrient_id를 포함시키고
#       애플리케이션 레벨에서 nutrients 테이블 조회 필요
# ============================================================
...
resource "aws_dms_replication_task" "analysis_to_chatbot" {
  ...
  table_mappings = jsonencode({
    rules = [
...

      {
        rule-type   = "transformation"
        rule-id     = "101"
        rule-name   = "rename-table-analysis-result"
        rule-action = "rename"
        rule-target = "table"
        object-locator = {
          schema-name = "public"
...

      {
        rule-type   = "transformation"
        rule-id     = "201"
        rule-name   = "rename-table-nutrient-gap"
        rule-action = "rename"
        rule-target = "table"
        object-locator = {
          schema-name = "public"
      ...
      },
      {
        rule-type   = "transformation"
        rule-id     = "204"
        rule-name   = "rename-current-amount"
        rule-action = "rename"
        rule-target = "column"
        object-locator = {
          schema-name = "public"
...

      {
        rule-type   = "transformation"
        rule-id     = "301"
        rule-name   = "rename-table-recommendations"
        rule-action = "rename"
        rule-target = "table"
        object-locator = {
          schema-name = "public"
      ...
      },
      {
        rule-type   = "transformation"
        rule-id     = "304"
        rule-name   = "rename-recommend-serving"
        rule-action = "rename"
        rule-target = "column"
        object-locator = {
          schema-name = "public"
          table-name  = "recommendations"
          column-name = "recommend_serving"
        }
  ...
  tags = {
    Name = "${local.name_prefix}-analysis-to-chatbot"
  }
}
